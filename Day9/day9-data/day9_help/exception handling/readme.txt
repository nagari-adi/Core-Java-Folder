...
Javac forces handling of the checked exceptions upon the programmer(Handling by supplying matching try-catch block or including it in the throws clause.)

Legal syntax
1. try {...} catch (exc1 e){...}
2. try {...} catch (exc1 e){...} catch (exc2 e) {..} ....
3. try {...} catch (exc1 e){} catch (exc2 e) {}catch(Exception e){catch-all}
3.5  try {..NPE.} catch (AE e){...} catch (NPE | SE e) {...}catch(Exception e){catch-all}
4. throws syntax ---
...
Mandatory--- only in case of un handled(no try-catch) chked excs.
